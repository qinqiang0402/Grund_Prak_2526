...
# ---------------------------------------------------------------------
# line plot of birthrate & female employment by district

data_joined <- left_join(birthrate, employment_female, 
                         by = c("Jahr", "Raumbezug")) %>%
...
plot_birthrate_district <- function(input.district) {
  ...
  if (input.district %in% districts) {
    ...
    # e.g.: "Lehel" instead of "Altstadt - Lehel"
    district.name <- districts[str_detect(districts, regex(input.district, 
                                                           ignore_case = TRUE))]
    }
  }
  ...
  if (length(district.name) > 1) {
    message("ℹ More than one district been found. Now plotting: ", 
            district.name[1])
