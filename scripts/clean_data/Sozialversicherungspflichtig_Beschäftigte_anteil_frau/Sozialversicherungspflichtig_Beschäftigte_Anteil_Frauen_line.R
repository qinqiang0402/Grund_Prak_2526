...
district_averages <- Sozialversicherungspflichtig_Beschäftigte_anteil_frau %>%
  group_by(Raumbezug) %>% 
...
data_for_plot <- Sozialversicherungspflichtig_Beschäftigte_anteil_frau %>%
  mutate(
...
ggplot(data = data_for_plot, mapping = aes(x = Jahr, y = anteil)) +
  ...
  labs(
    title = "Durchschnittliche Sozialversicherungspflichtig Beschäftigte Anteil von Frauen in München",
