...
GLOBAL_CORR <- combined_data_all %>%
  filter(!is.na(age_mean) & !is.na(anteil)) %>%
  summarise(corr = cor(age_mean, anteil)) %>%
  pull(corr)

OVERALL_CORR_FORMATTED <- paste0(
  "Gesamtkorrelation (", MIN_YEAR, " - ", MAX_YEAR, "): ", 
  round(GLOBAL_CORR, 3)
)
...
ui <- fluidPage(
  ...
  fluidRow(
    column(6,
           div(class = "map-title", "Karte 1: Anteil weiblicher Sozialversicherungspflichtig Beschäftigte"),
  ...
  fluidRow(
    column(12,
           ...
           div(class = "correlation-box", style = "text-align: center;",
               h4(textOutput("yearly_correlation"), style = "color: #004d99; font-weight: bold; margin-bottom: 5px;"),
               h4(OVERALL_CORR_FORMATTED, style = "color: #555; font-size: 16px;")
           )
    )
  )
)
...
server <- function(input, output, session) {
  ...
  yearly_correlation_value <- reactive({
    data_filtered <- combined_data_all %>%
      filter(Jahr == input$time_slider, !is.na(age_mean), !is.na(anteil))
    if (nrow(data_filtered) < 2) {
      return(paste0("Korrelation für Jahr ", input$time_slider, ": Zu wenig Datenpunkte"))
    }
    corr_val <- cor(data_filtered$age_mean, data_filtered$anteil)
    
    paste0(
      "Korrelationskoeffizient (r) für Jahr ", input$time_slider, ": ",
      round(corr_val, 3)
    )
  })
  output$yearly_correlation <- renderText({
    yearly_correlation_value()
  })
