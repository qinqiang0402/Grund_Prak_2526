...
ki_dual_trend <- readRDS("results/figures/NEW_Kinderbetreuung/ki_dual_trend.rds")
ki_trend_sv <- readRDS("results/figures/NEW_Kinderbetreuung/trend_sv.rds")
ki_map_2015 <- readRDS("results/figures/NEW_Kinderbetreuung/map_ki_2015.rds")
sv_map_2015 <- readRDS("results/figures/NEW_Kinderbetreuung/map_sv_2015.rds")
ki_point_korr_gesamt_sw <- readRDS("results/figures/NEW_Kinderbetreuung/ki_point_korr_gesamt_sw.rds")
ki_korr_jahre_point_line_rot <- readRDS("results/figures/NEW_Kinderbetreuung//ki_point_line_rot.rds")
ki_korr_stadtteile_point_line_sw <- readRDS("results/figures/NEW_Kinderbetreuung/ki_korr_stadtteile_point_line_sw.rds")
...
knitr::kable(table_data_01, format = "html", table.attr = 'class="no-wrap"')
```
...
knitr::kable(table_data_02, format = "html", table.attr = 'class="no-wrap"')
```
...
knitr::kable(table_data_03, format = "html", table.attr = 'class="no-wrap"')
```
...
<div style="text-align:left; font-size:2.0rem; margin-top:12px;">
...
observe({
  # 1) 只有在播放时才继续（目的：暂停时不跑、不注册定时器）
  req(playing_scatter())

  # 2) 定时器放在 req 后面（目的：避免暂停时也在后台 tick）
  invalidateLater(1000, session)

  # 3) 用 isolate 读取 year，并且也在 isolate 里做非空检查
  #    （目的：不建立对 input$year 的依赖，避免 updateSliderInput 立刻触发本 observe）
...
![](images/clipboard-742413736.png)
...
observe({
  ...
  # ② 定时器放在 req 后面
  # 目的：只有 playing 时才按固定间隔 tick
  invalidateLater(AUTO_MS5, session)

  # ③ 用 isolate 读取 year5，并在 isolate 内做非空检查
  # 目的：不建立对 input$year5 的反应式依赖，避免 updateSliderInput 造成“瞬间连跳”
  current <- isolate(input$year5)
  req(!is.null(current))

  # ④ 计算 next_year
  # 目的：严格按 year_seq5 推进，末尾回到开头
  ...
  df_merged %>%
    ...
    inner_join(
      df_betreut %>%
        ...
        select(Raumbezug, anteil_betreut),   # ⭐ 保持和原来列名一致
      by = "Raumbezug"
    )
})
...
df_year_grouped5 <- reactive({
  ...
  d %>%
    mutate(
      betreuung_group = cut(            # ⭐ 列名仍叫 betreuung_group，就和原版一样
        anteil_betreut,
        breaks = qs,
        labels = c("Niedrig", "Mittel", "Hoch"),
        include.lowest = TRUE
...
df_year_scaled5 <- reactive({
  df_year_grouped5()   # ⭐ 使用原始数据，不再 scale()
})
...
df <- df %>%
  ...
  filter(!is.na(gruppe))   # ⭐ 去掉 NA 分组

  ggplot(df) +

    # ⭐ 使用原始数据，不再用标准化后的 x_std5 / y_std5
    geom_point(
...
# ---- 一次性准备第 4 个 Shiny 用的数据（full_data_4）----
if (!exists("full_data_4", inherits = TRUE)) {
  ...
  # Kinderbetreuung bis 2 Jahre：总儿童数
  ...
  # Kinderbetreuung bis 2 Jahre：托管儿童数
...
observe({
  req(playing_4())
  invalidateLater(1000, session)   # 播放速度：1 秒一步
...
# -------------------------------------------------
# 左图：HK hoch + Beschäftigung niedrig 染粉色
# -------------------------------------------------
output$map_left_4 <- leaflet::renderLeaflet({
  ...
  # 这里先在 R 里生成 label，避免 leaflet 公式里用 !!
  popup_left_4 <- paste0(
...
# -------------------------------------------------
# -------------------------------------------------
# 右图：Kinderbetreuung 红色渐变 + 小号 Legend
# -------------------------------------------------
output$map_right_4 <- leaflet::renderLeaflet({
  ...
  leaflet::leaflet(d) %>%
    ...
    leaflet::addLegend(
      ...
      position = "bottomright",
      className = "small-legend"   # ⭐ 给 legend 加一个 CSS class
    ) %>%
    htmlwidgets::prependContent(  # ⭐ 用内联 CSS 把 legend 缩小
