...
# Load Map
munich_map_sf <- st_read("results/geo/bezirk_map.json", quiet = TRUE)

# Load R scripts
#SV
source("scripts/Frauenbeschaefitung/SV_Durchschnitte_Leaflet_new.R")
#HMK
source("scripts/Haushalte_mit_Kindern/HMK_Gesamt.R")
#KI
source("scripts/Kinderbetreuung/KI_dual_Trend_test.R")
source("scripts/Kinderbetreuung/ki_korr_gesamt&Stadtteile_test.R")
source("scripts/Kinderbetreuung/korr_jahr_rot_test.R")
source("scripts/Kinderbetreuung/ki_map_2015_test.R")
source("scripts/Kinderbetreuung/sv_map_2015_test.R")
#m_effect
source("scripts/Moderierender_Effekt/m_effect_correlation_group_new.R")
source("scripts/Moderierender_Effekt/m_effect_district_group_2024_new.R")
source("scripts/Moderierender_Effekt/m_effect_district_Kinderbetreuung_new.R")
source("scripts/Moderierender_Effekt/m_effect_scatter_2024_new.R")
source("scripts/Anhang/Anhang_moderierender_Effekt/Kinderbetreuung_2024_leaflet_new.R")
source("scripts/Anhang/Anhang_moderierender_Effekt/m_effect_district_group_2024_leaflet_new.R")
#Geburtenrate
source("scripts/Anhang/Geburtenrate/br_korr_new.R")
source("scripts/Anhang/Geburtenrate/line_br_emp_new.R")
source("scripts/Anhang/Geburtenrate/line_br_new.R")
#MEA
source("scripts/Anhang/Mutter_Erstgeburt_Alter/MEA_Korr_nach_Jahr_new.R")
source("scripts/Anhang/Mutter_Erstgeburt_Alter/MEA_Korr_nach_Stadtteilen_new.R")
source("scripts/Anhang/Mutter_Erstgeburt_Alter/MEA_Linien_new.R")
#tables
source("scripts/tables/tables.R")
tables_list <- generate_tables(ar_sheet, be_sheet, ki_sheet)
table_data_01 <- tables_list$t1
table_data_02 <- tables_list$t2
table_data_03 <- tables_list$t3
...
#SV
leaflet_sv_durchschnitt <- generate_emp_deviation_map(ar_sheet, munich_map_sf)
#HMK
hmk_plots_list <- generate_hmk_plots(ar_sheet, be_sheet)
#KI
ki_dual_trend_test <- generate_ki_dual_trend(ar_sheet, be_sheet, ki_sheet)
ki_corr_plots <- generate_ki_corr_plots(ar_sheet, be_sheet, ki_sheet)
ki_korr_jahre_point_line_rot_test <- generate_ki_year_trend(ar_sheet, be_sheet, ki_sheet)
ki_map_2015_test <- generate_ki_map(be_sheet, ki_sheet, munich_map_sf, target_year = 2015)
sv_map_2015_test <- generate_emp_map(ar_sheet, munich_map_sf, target_year = 2015)
#m_effect
m_effekt_kor_group <- generate_moderation_plot(ar_sheet, be_sheet, ki_sheet)
m_effekt_04_plot <- generate_target_group_map(ar_sheet, be_sheet, munich_map_sf)
m_effekt_03_plot <- generate_childcare_map_2024(be_sheet, ki_sheet, munich_map_sf)
m_effekt_2.5 <- generate_target_group_scatter(ar_sheet, be_sheet)
m_effekt_03_leaflet <- generate_childcare_leaflet_2024(be_sheet, ki_sheet, munich_map_sf)
m_effekt_04_leaflet <- generate_target_group_leaflet(ar_sheet, be_sheet, munich_map_sf)
#Geburtenrate
birthrate_plots <- generate_birthrate_correlation_plots(ar_sheet, be_sheet)
geburtenrate_dual_trend <- generate_birthrate_trend(ar_sheet, be_sheet, "Stadt München")
geburtenrate_trend_nach_stadtteile <- generate_birthrate_district_trend(be_sheet)
#MEA
mea_year_plots <- generate_mea_year_plots(ar_sheet, be_sheet)
mea_district_plots <- generate_mea_district_plots(ar_sheet, be_sheet)
mea_trend_plot <- generate_mea_trend_plot(be_sheet)

# Extract individual plots for convenience
#KI
ki_point_korr_gesamt_sw_test      <- ki_corr_plots$gesamt
ki_korr_stadtteile_point_line_sw_test <- ki_corr_plots$stadtteile
#HMK
hmk_korr_gesamt_point_sw <- hmk_plots_list$gesamt
hmk_korr_point_line_nach_jahr_color <- hmk_plots_list$jahr
hmk_korr_point_line_nach_stadtteile_color <- hmk_plots_list$stadtteile
#Geburtenrate
geburtenrate_korr_nach_stadtteile  <- birthrate_plots$stadtteile
geburtenrate_korr_trend_nach_jahr  <- birthrate_plots$trend_jahr
#MEA
mea_korr_jahr_point      <- mea_year_plots$point_plot
mea_korr_jahr_point_line <- mea_year_plots$line_plot
mea_gesamt_sw                        <- mea_district_plots$gesamt
mea_korr_nach_stadtteilen_point      <- mea_district_plots$point_plot
mea_korr_nach_stadtteilen_point_line <- mea_district_plots$line_plot
...
ar_sheet <- ar_sheet %>% mutate(Jahr = as.integer(Jahr))
be_sheet <- be_sheet %>% mutate(Jahr = as.integer(Jahr))
ki_sheet <- ki_sheet %>% mutate(Jahr = as.integer(Jahr))

munich_map <- munich_map_sf <- st_read("results/geo/bezirk_map.json", quiet = TRUE) |> st_transform(4326)
...
#| echo: false
leafletOutput("map_frauen_static", height = "500px", width = "100%")
...
output$map_frauen_static <- renderLeaflet({
...
ki_korr_stadtteile_point_line_sw_test
```
...
ki_korr_jahre_point_line_rot_test
```
...
```{r}
#| echo: false
leafletOutput("map_target_group", height = "400px", width = "100%")
```
:::
...
```{r}
#| echo: false
leafletOutput("map_childcare_rates", height = "400px", width = "100%")
```
:::
:::::
...
output$map_target_group <- renderLeaflet({
  m_effekt_04_leaflet
})


output$map_childcare_rates <- renderLeaflet({
  m_effekt_03_leaflet
})
